...
current_brands = ['oppo','vivo','realme','xiaomi','samsung']

while True:
    for individual_brand in current_brands:

        email = Email.objects.filter(brand=individual_brand,processed=False,rejected=False).order_by('created_at').first()
        if email:
        ...
        for attachment in attachments:
            ...
            if not attachment.attachment.name.lower().endswith(".xlsx"):
                continue

            context_data = read_file_data(BASE_DIR/"engine_brain"/f"{individual_brand}_explainer.txt")
            sparcs_fields = read_file_data(BASE_DIR/"engine_brain"/f"{individual_brand}_sparcs_fields.txt")
            sample_scheme_chunck = read_file_data(BASE_DIR/"engine_brain"/f"{individual_brand}_sample_scheme_chunck.txt")
                    ...
                    for sheet_name in sheet_list:
                        ...
                        for row in df:
                            try:
                                ...
                                new_row = Input_Row.objects.create(brand=individual_brand,jsonified_row_data = input_data)
